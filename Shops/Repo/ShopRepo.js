...
module.exports.viewAllShops = async (params = {}) => {
    try {
        const {
            page = 1,
            limit = 10,
        ...
        // Build filter query - Only show verified and active shops to public
        const matchQuery = { 
            isActive: true, 
            isVerified: true 
        };
        ...
        const sortObj = {};
        if (sort === 'name') {
            sortObj.ShopName = order === 'asc' ? 1 : -1;
        } else {
            sortObj[sort] = order === 'asc' ? 1 : -1;
        }

        let shops, totalShops;
        ...
        if (lat && lng) {
            const pipeline = [
                {
                    $geoNear: {
                        ...
                        spherical: true,
                        query: matchQuery // Apply city filter
                    }
                }
            ];

            // Only sort if a sort was explicitly requested, otherwise it defaults to nearest first.
            // If sort is requested, the $geoNear sorting by distance is overridden.
            if (sort !== 'createdAt') {
            ...
            pipeline.push({
                $addFields: {
                    ...
                    distanceText: { $concat: [{ $toString: { $round: ["$distance", 1] } }, " km"] }
                }
            });

            const [aggregateResult, countResult] = await Promise.all([
                ShopModel.aggregate(pipeline),
                ShopModel.aggregate(countPipeline)
            ]);

            shops = aggregateResult;
            totalShops = countResult.length > 0 ? countResult[0].total : 0;

        } else {
            // Standard query
            const [findResult, countResult] = await Promise.all([
                ShopModel.find(matchQuery)
                    .sort(sortObj)
                    .skip(skip)
                    .limit(limit)
                    .lean(),
                ShopModel.countDocuments(matchQuery)
            ]);
            shops = findResult;
            totalShops = countResult;
        }
...
module.exports.viewAllShopsForAdmin = async (params = {}) => {
    try {
        const {
            page = 1,
            limit = 10,
        ...
        const [shops, totalShops] = await Promise.all([
            ShopModel.find(matchQuery)
                ...
                .skip(skip)
                .limit(limit)
                .populate('ShopOwnerId', 'firstName lastName email Mobile')
                .lean(),
            ShopModel.countDocuments(matchQuery)
        ]);
        ...
        console.error("Error in viewAllShopsForAdmin:", error);
        throw error;
    }
}
        ...
        const shops = await ShopModel.aggregate([
            {
                $geoNear: {
                    ...
                    query: { 
                        isActive: true,
                        isVerified: true
                    } // ✅ Only show verified and active shops during geo search
                }
            },
            { $limit: 50 }
        ]);
...
module.exports.verifyShopFunction = async (shopId, status) => {
    try {
        ...
        console.error("Error in verifyShopFunction:", error);
        throw error;
    }
}
