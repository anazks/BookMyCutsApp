...
module.exports.addBookings = async (data) => {
  try {
    ...
    logger.info("New Booking Created", { booking: bookings });
    return bookings
  } catch (error) {
...
module.exports. findDashboardIncomeFunction = async (shopId) => {
  ...
  try {
    ...
    logger.info('[DASHBOARD] Total bookings for shop', { shopId, totalBookings });

    const validStatusBookings = await BookingModel.countDocuments(baseQuery);
    logger.info('[DASHBOARD] Valid status bookings', { count: validStatusBookings });

    if (totalBookings === 0) {
      logger.info('🚫 [DASHBOARD] NO bookings exist for this shop');
