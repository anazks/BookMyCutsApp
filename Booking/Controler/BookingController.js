...
const AddBooking = async (req, res) => {
  try {
    logger.info("AddBooking request received", { body: req.body });
    let token = req.headers['authorization']?.split(' ')[1];
    let decodedValue = await Decoder(token);
    logger.info("Token decoded for AddBooking", { decodedValue });
    
    let BookingStatus = await bookNow(req.body, decodedValue);
    logger.info("Booking process completed", { BookingStatus });
    return res.status(200).json({ success: true, BookingStatus });
  } catch (error) {
    logger.error("AddBooking error", { error: error.message });
...
const createOrder = async (req, res) => {
  try {
    ...
    }).catch(err => logger.error("❌ Failed to log ORDER_CREATED stage", { error: err.message }));

      logger.info("Payment details before response", {
...
const verifyPayment = async (req, res) => {
  logger.info("🔍 Verify payment started...", { body: req.body });
  try {
    const {
      razorpay_payment_id,
      razorpay_order_id,
    ...
    if (!existingEarning) {
      ...
      logger.info(`✅ Ledger Entry Created: Service ₹${totalServicePrice} | Bonus ₹${salonBonus} | Total Entry: ₹${salonPayoutAmount}`);
    }
    // --- PAYOUT & LEDGER LOGIC END ---

    logger.info("✅ Payment Verification Successful", { orderId: razorpay_order_id, paymentId: razorpay_payment_id });
    await TransactionLog.create({
      bookingId: bookingId,
